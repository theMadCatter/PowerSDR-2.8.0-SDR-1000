			 ...
			 char *tag)     // String to emit when error conditions occur
{
	...
	// systems is determined from the calculations we just made.
	// The fast track agc is designed to prevent large pulse signals from pushing
	// us into serious attack and waiting for a long decay when the signal is short duration (<3 time constants) for attack

	// Compute exponential fast attack rate per sample from fast attack time and sampler rate.
	a->fastattack = (REAL) (1.0 - exp (-1000.0 / (0.2 * samprate))); // 0.2 ms
	...
	a->fastdecay = (REAL) (1.0 - exp (-1000.0 / (3.0 * samprate))); // 3.0 ms
...
DttSPAgc (DTTSPAGC a, int tick)
{
	...
	// if AGC is off, just apply the fixed gain to all samples
	if (a->mode == agcOFF)
	{
	...
	hangtime = (int) (uni[0].samplerate * a->hangtime); // hangtime in samples
	fasthangtime = (int) (uni[0].samplerate * a->fasthangtime); // fast track hangtime in samples

//	if (a->hangthresh > 0)
		hangthresh =
			a->gain.top * a->hangthresh + a->gain.bottom * (1.0f - a->hangthresh);
//	else
//		hangthresh = 0.;	
